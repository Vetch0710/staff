...
let setWatermarkContent = (obj,content) => {
...
// 该方法只允许调用一次
watermarkOption.set = (obj,content) => {
  let id = setWatermarkContent(obj,content)
  setInterval(() => {
    if (document.getElementById(id) === null) {
      id = setWatermarkContent(obj,content)
    }
  }, 500)
  obj.onresize = () => {
    setWatermarkContent(obj,content)
  }
}
